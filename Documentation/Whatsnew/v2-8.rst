...
The setting for the autocompletion of the built in Sphinx offline search no is
`disableAutoSelect = true`. This means, that the search will be done for
exactly the content of the search field when you press enter.
...
.. versionchanged:: 2.8.1 Show constantly
   ...
   not shown though if there is no "Edit on GitHub"-like button at all.
